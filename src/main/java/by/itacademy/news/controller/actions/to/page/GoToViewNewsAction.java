    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (!contentChecker.isEmpty(id)) {
                ...
                doResponse(request, ParameterType.PRESENTATION.getParameter(), ParameterType.VIEW_NEWS.getParameter(),
                        PathType.BASE_LAYOUT.getPath(), response);
            } else {
                doResponse(request, ParameterType.EXCEPTION_MSG.getParameter(), OutputMessage.WRONG_MSG.getMessage(),
                        PathType.ERROR_PAGE.getPath(), response);
            }

        } catch (NewsServiceException | NumberFormatException e) {
            doResponse(request, ParameterType.EXCEPTION_MSG.getParameter(), e.getMessage(), PathType.ERROR_PAGE.getPath(), response);
        }
    }
